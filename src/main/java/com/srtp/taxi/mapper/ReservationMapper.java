...
public interface ReservationMapper {
    ...
    @Select("select * from t_reservation where isDispatched=false and hour(timediff(startAt,now()))>=0 and hour(timediff(startAt,now()))<=8")
    List<Reservation> queryReservationNotDispatchedByInterval8();
}
